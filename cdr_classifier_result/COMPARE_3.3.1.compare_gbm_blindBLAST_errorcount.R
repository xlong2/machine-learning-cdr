...
conf_tables_all_loops_gbm_diff = lapply(merged_testing_result,function(x){x = x[as.character(x[,1])!=as.character(x[,2]),]; 
  x$Var1=gsub("_","-",x$Var1); x$Var2 = gsub("_","-",x$Var2); return(x)})
for(loop in names(conf_tables_all_loops_blindBLAST_diff)){
  ...
  blind_r[,1:2]=lapply(blind_r[,1:2],as.character)
  gbm_r[,1:2] =lapply(gbm_r[,1:2],as.character)
