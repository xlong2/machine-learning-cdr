# read in all rmsd files for data retrieval
...
L1_H1_names=sapply(strsplit(as.character(unlist(read.table(name_file))),"\\/"),"[[",7)
...
L1_index=which(grepl(x_type,marks)); L1_names=pdbs[L1_index];
...
colnames(L1_matrix)=L1_names[undup_L1_names_ind]; rownames(L1_matrix)=L1_names[undup_L1_names_ind]
all_rmsd_list[[x_type]]=L1_matrix
}
}

saveRDS(all_rmsd_list,file="./proline_classifier/Data_processed/all_rmsd_list.rds")
