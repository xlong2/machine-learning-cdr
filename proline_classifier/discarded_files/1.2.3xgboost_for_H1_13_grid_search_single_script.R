# try out xgboost to solve for the H1_13 grid searching problem. 
library("caret")
library("xgboost")
library(doParallel)
print("line 2")
...
if(file.exists(to_save_file)){
  print("file_already_exists!")}else{
    ...
    even_out_all_classes_non_cases_no_repeat<-function(training_cases){
      ...
      max_class=names(proportions)[which(proportions==max(proportions))][1]
    ...
    generic_train<-function(each_loop_length_data_feature_string_rmsd,each_method,training_cases){
      ...
      if(test_jazz){
        ...
        single_grid_test=as.data.frame(list(nrounds=200,min_child_weight=1, gamma=0,max_depth=10, eta=0.1, colsample_bytree=0.5, subsample=0.5))
        
        trained_model <- train(each_loop_length_data_feature_string_rmsd, data = training_cases,
                               method = "xgbTree",   # fold number 10
                               trControl = fitcontrol_test,
                               tuneGrid = single_grid_test,
                               verbose=TRUE,
                               ## Specify which metric to optimize
                               metric = "kappa",ncores=3)
        ...
        the_grid=as.data.frame(list(nrounds=as.numeric(n.trees),min_child_weight=1, gamma=0,max_depth=interaction.depth, eta=shrinkage, colsample_bytree=0.5, subsample=0.5))
        ...
        fitControl <- trainControl(method = "repeatedcv",
                                   ...
                                   summaryFunction = multiClassSummary)
        trained_model=""
        #fitControl <- trainControl(method =NULL,classProbs = TRUE, savePredictions="all")
        trained_model <- train(each_loop_length_data_feature_string_rmsd, data = training_cases,
